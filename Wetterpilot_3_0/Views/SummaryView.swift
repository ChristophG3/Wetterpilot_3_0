...
struct SummaryView: View {
    ...
    var body: some View {
        List {
            if let best = vm.summary.max(by: { a, b in a.scoreOpen < b.scoreOpen }) {
    ...
    private func card(for item: SummaryItem) -> some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                Text(DF.display(isoKey: item.key))
                    .font(.headline)
                    .monospacedDigit()
            ...
            Spacer()
            // Offener Score groß • 0–100 klein daneben
            VStack(alignment: .trailing, spacing: 4) {
                Text("\(item.scoreOpen)")
                    .font(.title3).bold()     // offene Skala
                Text("\(item.scoreDisplay)/100")
                    .font(.footnote)
